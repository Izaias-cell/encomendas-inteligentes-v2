    ...
    <div className="p-6 max-w-4xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="space-y-2">
          ...
          <p className="text-sm text-zinc-500">
            Ajustes visuais e de comportamento do sistema.
          </p>
        </div>
        ...
        <div className="md:col-span-2 bg-white rounded-3xl border border-zinc-100 p-8 shadow-sm space-y-6">
          <div className="flex items-center justify-between p-4 bg-zinc-50 rounded-2xl border border-zinc-100">
            <div>
              <p className="font-bold text-zinc-900">Modo Claro (Padrão)</p>
              <p className="text-xs text-zinc-500">Manter cores claras e vibrantes</p>
            </div>
            <button 
              onClick={() => setSettings({...settings, light_mode_enabled: !settings.light_mode_enabled})}
              className={`w-12 h-6 rounded-full transition-all relative ${settings.light_mode_enabled ? 'bg-emerald-500' : 'bg-zinc-300'}`}
            >
              <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${settings.light_mode_enabled ? 'right-1' : 'left-1'}`} />
            </button>
          </div>
        </div>
      </div>
