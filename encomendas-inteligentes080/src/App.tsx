...
const LoginPage = ({ onLogin }: any) => {
  ...
  const handleConfirmPortariaSession = useCallback(async (permanent: boolean) => {
    ...
    try {
      ...
      // Limpa qualquer sessão residual do Supabase Auth no dispositivo local antes de ativar a portaria
      try {
        await supabase.auth.signOut({ scope: 'local' });
      } catch (signOutErr) {
        console.warn('[App] Aviso ao limpar sessão local do Supabase Auth:', signOutErr);
      }
