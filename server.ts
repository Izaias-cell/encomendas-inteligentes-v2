...
const app = express();
  ...
  // --- COMMERCIAL DEMO QR & REFERRAL (15+15) ENGINE ---
  interface DemoQrRecord {
    id: string;
    token: string;
    origin: 'ADMINISTRADOR' | 'SINDICO';
    created_by?: string;
    created_at: string;
  ...
  interface ReferralLeadRecord {
    ...
    token_or_ref: string;
    status: 'INTERESSE' | 'TESTE_ADERIDO' | 'TESTE_ATIVADO';
  ...
  const demoQrStore = new Map<string, DemoQrRecord>();
  const referralLeadsStore = new Map<string, ReferralLeadRecord>();
  const condoTrialStore = new Map<string, {
  ...
  // Ensure default Admin Demo QR exists
  const defaultAdminDemoToken = 'DEMO-OFICIAL';
  if (!demoQrStore.has(defaultAdminDemoToken)) {
    demoQrStore.set(defaultAdminDemoToken, {
      id: 'demo-qr-default',
      token: defaultAdminDemoToken,
      origin: 'ADMINISTRADOR',
      created_by: 'Administrador Master',
      created_at: new Date().toISOString(),
      status: 'active',
      hits_count: 0,
      adhesions_count: 0,
      url: `/demonstracao?token=${defaultAdminDemoToken}`
    });
  }

  const generateReferralCodeForCondo = (condoName: string, condoId: string): string => {
    const clean = (condoName || 'CONDO')
      .toUpperCase()
      .normalize("NFD").replace(/[\u0300-\u036f]/g, "")
      .replace(/^CONDOMINIO\s+/i, '')
      .replace(/[^A-Z0-9]/g, '');
    const prefix = clean.length >= 4 ? clean.substring(0, 4) : 'SIND';
    const suffix = condoId ? condoId.replace(/-/g, '').substring(0, 4).toUpperCase() : Math.floor(1000 + Math.random() * 9000).toString();
    return `REF-${prefix}-${suffix}`;
  };

  const getOrInitCondoTrial = async (condoId: string, condoName: string) => {
    if (condoTrialStore.has(condoId)) {
      return condoTrialStore.get(condoId)!;
    }
    ...
    try {
      ...
      if (condo) {
        const now = new Date();
        const defaultStart = condo.created_at || now.toISOString();
        const defaultEnds = new Date(new Date(defaultStart).getTime() + 15 * 24 * 60 * 60 * 1000).toISOString();

        const trialInfo = {
          trial_started_at: condo.trial_started_at || defaultStart,
          trial_ends_at: condo.trial_ends_at || defaultEnds,
          plan_status: (condo.plan_status as any) || 'trial',
          referral_code: condo.referral_code || generateReferralCodeForCondo(condo.name, condo.id),
          referral_bonus_days: Number(condo.referral_bonus_days || 0)
        };

        condoTrialStore.set(condoId, trialInfo);

        // Best-effort update to DB if columns exist
        (async () => {
          try {
            await supabaseAdmin.from('condominiums').update({
              trial_started_at: trialInfo.trial_started_at,
              trial_ends_at: trialInfo.trial_ends_at,
              plan_status: trialInfo.plan_status,
              referral_code: trialInfo.referral_code,
              referral_bonus_days: trialInfo.referral_bonus_days
            }).eq('id', condoId);
          } catch (err) {
      ...
      console.warn("[Trial Engine] DB fetch warning:", e);
    }

    const now = new Date();
    const trialInfo = {
      trial_started_at: now.toISOString(),
      trial_ends_at: new Date(now.getTime() + 15 * 24 * 60 * 60 * 1000).toISOString(),
      plan_status: 'trial' as const,
      referral_code: generateReferralCodeForCondo(condoName, condoId),
...
*Encomendas Inteligentes*`;
  ...
  // =========================================================================
  // --- COMMERCIAL MECHANISM: DEMO QR, REFERRAL PROGRAM & 15+15 TRIAL ENGINE ---
  // =========================================================================

  // 1. Admin: Generate New Demo QR Code
  app.post("/api/admin/demo-qr/generate", async (req, res) => {
    const session = await validateAdminSession(req);
    if ("error" in session) return res.status(session.status).json({ error: session.error });
    const { adminUser, adminProfile } = session;

    try {
      const token = `DEMO-ADM-${crypto.randomBytes(3).toString('hex').toUpperCase()}`;
      const id = `qr-${Date.now()}-${Math.floor(1000 + Math.random() * 9000)}`;
      const baseUrl = getCanonicalPublicBaseUrl(req);
      const fullUrl = `${baseUrl}/demonstracao?token=${token}`;

      const qrRecord: DemoQrRecord = {
        id,
        token,
        origin: 'ADMINISTRADOR',
        created_by: adminProfile.full_name || adminUser.email || 'Administrador',
        created_at: new Date().toISOString(),
        status: 'active',
        hits_count: 0,
        adhesions_count: 0,
      ...
      demoQrStore.set(token, qrRecord);

      await safeLogAuditoria({
        condominio_id: null,
        usuario_id: adminProfile.id,
        usuario_nome: adminProfile.full_name,
        usuario_perfil: adminProfile.role,
        tipo_evento: 'QR_DEMO_CRIADO',
        acao: 'INSERT',
        tabela_afetada: 'demonstracao_qrs',
        registro_id: id,
        descricao: `Link Oficial de Demonstração gerado pelo Administrador (${token}).`,
        metodo: 'ADMIN_ACTION',
        dados_depois: qrRecord
      });

      return res.json({ success: true, qr: qrRecord });
    } catch (err: any) {
      console.error("[Demo QR] Erro ao gerar QR Code:", err);
      return res.status(500).json({ error: err.message || "Erro interno ao gerar link de demonstração." });
    }
  });

  // 2. Admin: List Generated Demo QR Codes
  app.get("/api/admin/demo-qr", async (req, res) => {
    ...
    try {
      const baseUrl = getCanonicalPublicBaseUrl(req);
      const list = Array.from(demoQrStore.values())
        .map(item => ({
          ...item,
          url: `${baseUrl}/demonstracao?token=${item.token}`
        }))
        .sort((a, b) => 
  ...
  // 3. Public: Get Demo Information by Token or Ref (No Auth Required)
  app.get("/api/public/demo-info", async (req, res) => {
    ...
    try {
      let origin_type: 'ADMINISTRADOR' | 'SINDICO' = 'ADMINISTRADOR';
      let origin_title = 'Demonstração Oficial';
      let referrer_name = 'Administração Encomendas Inteligentes';
      let condo_name = '';
      let referrer_condo_id = '';
      let referrer_user_id = '';

      if (ref) {
        // Search in-memory and database for matching condominium referral code
        let matchedCondo: any = null;
        for (const [cId, trial] of condoTrialStore.entries()) {
          if (trial.referral_code && trial.referral_code.toUpperCase() === ref) {
            const { data: dbC } = await supabaseAdmin.from('condominiums').select('*').eq('id', cId).maybeSingle();
            matchedCondo = dbC || { id: cId, name: 'Condomínio Parceiro' };
            break;
          }
        }

        if (!matchedCondo) {
          const { data: dbCondos } = await supabaseAdmin.from('condominiums').select('*');
          if (dbCondos) {
            for (const c of dbCondos) {
              const trial = await getOrInitCondoTrial(c.id, c.name);
              if (trial.referral_code.toUpperCase() === ref) {
                matchedCondo = c;
                break;
              }
            }
        ...
        if (matchedCondo) {
          origin_type = 'SINDICO';
          origin_title = `Indicação de Síndico Parceiro (${matchedCondo.name})`;
          condo_name = matchedCondo.name;
          referrer_condo_id = matchedCondo.id;

          // Find sindico profile name
          const { data: sindicoProfile } = await supabaseAdmin
            .from('profiles')
            .select('id, full_name')
            .eq('condominium_id', matchedCondo.id)
            .eq('role', 'sindico')
            .maybeSingle();

          if (sindicoProfile) {
            referrer_name = `Síndico ${sindicoProfile.full_name}`;
            referrer_user_id = sindicoProfile.id;
          } else {
            referrer_name = `Síndico do ${matchedCondo.name}`;
          }
        }
      } else if (token && demoQrStore.has(token)) {
        const qr = demoQrStore.get(token)!;
        qr.hits_count += 1;
        demoQrStore.set(token, qr);
      }

      // Log audit event for demo access
      await safeLogAuditoria({
        condominio_id: referrer_condo_id || null,
        usuario_nome: 'Visitante Público',
        usuario_perfil: 'lead',
        tipo_evento: 'DEMO_ACESSADA',
        acao: 'SELECT',
        tabela_afetada: 'demonstracao',
        descricao: `Demonstração pública acessada via ${origin_type === 'SINDICO' ? `indicação do ${condo_name} (${ref})` : `QR Code Admin (${token || 'PADRÃO'})`}.`,
        metodo: 'PUBLIC_ACCESS',
        dados_depois: { origin_type, origin_title, referrer_name, condo_name, token, ref }
      });
      ...
      console.error("[Demo Info] Erro ao buscar informações da demonstração:", err);
      return res.status(500).json({ error: "Erro ao processar dados da demonstração." });
    }
  });

  // 4. Public: Submit Adherence Form ("QUERO ADERIR AO TESTE")
  app.post("/api/public/demo-adhere", async (req, res) => {
    const { name, phone, condo_name, token, ref } = req.body;

    if (!name || typeof name !== 'string' || !name.trim()) {
      return res.status(400).json({ error: "Por favor, informe seu nome completo." });
    }
    if (!phone || typeof phone !== 'string' || !phone.trim()) {
      return res.status(400).json({ error: "Por favor, informe seu WhatsApp de contato." });
    }
    if (!condo_name || typeof condo_name !== 'string' || !condo_name.trim()) {
      return res.status(400).json({ error: "Por favor, informe o nome do seu condomínio." });
    }

    const cleanName = name.trim();
    const cleanPhone = phone.trim().replace(/\D/g, '');
    const cleanCondo = condo_name.trim();
    const cleanRef = typeof ref === 'string' ? ref.trim().toUpperCase() : '';
    const cleanToken = typeof token === 'string' ? token.trim().toUpperCase() : '';

    try {
      let origin_type: 'ADMINISTRADOR' | 'SINDICO' = 'ADMINISTRADOR';
      let referrer_condo_id = '';
      let referrer_condo_name = '';
      let referrer_user_id = '';
      let referrer_user_name = '';
      ...
      if (cleanRef) {
        ...
        if (allCondos) {
          for (const c of allCondos) {
            const trial = await getOrInitCondoTrial(c.id, c.name);
            if (trial.referral_code.toUpperCase() === cleanRef) {
              origin_type = 'SINDICO';
              referrer_condo_id = c.id;
              referrer_condo_name = c.name;

              const { data: sProfile } = await supabaseAdmin
                .from('profiles')
                .select('id, full_name')
                .eq('condominium_id', c.id)
                .eq('role', 'sindico')
                .maybeSingle();

              if (sProfile) {
                referrer_user_id = sProfile.id;
                referrer_user_name = sProfile.full_name;
              }
              break;
            }
          }
      ...
      // Check for duplicate pending lead (anti-spam / idempotency)
      let existingLeadId: string | null = null;
      for (const [lId, l] of referralLeadsStore.entries()) {
        const lPhoneClean = l.phone.replace(/\D/g, '');
        if (lPhoneClean === cleanPhone && l.condo_name.toLowerCase() === cleanCondo.toLowerCase()) {
          existingLeadId = lId;
          break;
        }
      }

      const leadId = existingLeadId || `lead-${Date.now()}-${Math.floor(1000 + Math.random() * 9000)}`;
      const leadRecord: ReferralLeadRecord = {
        id: leadId,
        name: cleanName,
        phone: phone.trim(),
        condo_name: cleanCondo,
        origin_type,
        referrer_condo_id: referrer_condo_id || undefined,
        referrer_condo_name: referrer_condo_name || undefined,
        referrer_user_id: referrer_user_id || undefined,
        referrer_user_name: referrer_user_name || undefined,
        token_or_ref: cleanRef || cleanToken || 'DEMO-OFICIAL',
      ...
      referralLeadsStore.set(leadId, leadRecord);

      // Increment adhesions on Admin QR if applicable
      if (cleanToken && demoQrStore.has(cleanToken)) {
        const qr = demoQrStore.get(cleanToken)!;
        qr.adhesions_count += 1;
        demoQrStore.set(cleanToken, qr);
      }

      // Log 1: TESTE_ADERIDO in audit trail
      await safeLogAuditoria({
        condominio_id: referrer_condo_id || null,
        usuario_nome: cleanName,
        usuario_perfil: 'lead',
        tipo_evento: 'TESTE_ADERIDO',
        acao: 'INSERT',
        tabela_afetada: 'referrals',
        registro_id: leadId,
        descricao: `Novo interesse registrado: ${cleanName} (${cleanCondo}, WhatsApp: ${phone.trim()}). Origem: ${origin_type === 'SINDICO' ? `Indicação de ${referrer_condo_name}` : 'QR Code Administrador'}.`,
        metodo: 'DEMO_ADHERENCE',
        dados_depois: leadRecord
      });

      // Log 2: ADMIN_NOTIFICADO in audit trail
      await safeLogAuditoria({
        condominio_id: referrer_condo_id || null,
        usuario_nome: 'Sistema de Notificação',
        usuario_perfil: 'sistema',
        tipo_evento: 'ADMIN_NOTIFICADO',
        acao: 'NOTIFY',
        tabela_afetada: 'referrals',
        registro_id: leadId,
        descricao: `Notificação enviada ao Administrador: Síndico ${cleanName} do condomínio ${cleanCondo} aderiu ao teste de 15 dias. Contato: ${phone.trim()}.`,
        metodo: 'SYSTEM_NOTIFICATION',
        dados_depois: {
          sindico_nome: cleanName,
          condominio_nome: cleanCondo,
          contato_whatsapp: phone.trim(),
          origem: origin_type === 'SINDICO' ? `Indicação do condomínio ${referrer_condo_name}` : 'Demonstração Direta',
          status: 'TESTE_ADERIDO',
      ...
      return res.json({
        success: true,
        leadId,
        message: "Parabéns! Sua adesão ao período de testes foi registrada com sucesso. Nossa equipe entrará em contato para ativar o seu condomínio com 15 dias grátis."
      });
    } catch (err: any) {
      console.error("[Demo Adhere] Erro ao registrar adesão:", err);
      return res.status(500).json({ error: err.message || "Erro interno ao registrar adesão ao teste." });
    }
  });

  // 5. Síndico: Get "Indique um Síndico" Referral Information & Stats
  app.get("/api/sindico/referral-info", async (req, res) => {
    const session = await validateAdminSession(req);
    if ("error" in session) return res.status(session.status).json({ error: session.error });
    const { adminProfile: profile } = session;

    if (!profile.condominium_id) {
      return res.status(400).json({ error: "Usuário não está vinculado a nenhum condomínio." });
    }

    try {
      const condoId = profile.condominium_id;
      const { data: condo } = await supabaseAdmin.from('condominiums').select('*').eq('id', condoId).maybeSingle();
      const condoName = condo?.name || 'Seu Condomínio';

      const trialInfo = await getOrInitCondoTrial(condoId, condoName);

      // Calculate remaining trial days
      const now = Date.now();
      const endsAt = new Date(trialInfo.trial_ends_at).getTime();
      const diffMs = endsAt - now;
      const daysRemaining = Math.max(0, Math.ceil(diffMs / (1000 * 60 * 60 * 24)));

      // Collect all leads referred by this condominium
      const referrals: ReferralLeadRecord[] = [];
      for (const lead of referralLeadsStore.values()) {
        if (lead.referrer_condo_id === condoId) {
          referrals.push(lead);
        }
      }
      ...
      const adheredCount = referrals.filter(r => r.status === 'TESTE_ADERIDO' || r.status === 'TESTE_ATIVADO').length;
      const activatedCount = referrals.filter(r => r.status === 'TESTE_ATIVADO').length;
      const totalBonusDays = trialInfo.referral_bonus_days || (activatedCount * 15);

      const baseUrl = getCanonicalPublicBaseUrl(req);
      const referralUrl = `${baseUrl}/demonstracao?ref=${trialInfo.referral_code}`;

      const shareText = `Olá, Síndico! Estou usando o sistema ENCOMENDAS INTELIGENTES no ${condoName} e a gestão da portaria ficou 100% automatizada e sem extravios.\n\nExperimente você também com 15 DIAS GRÁTIS pelo link:\n${referralUrl}`;
      ...
      return res.json({
        ...
        plan_status: trialInfo.plan_status,
        referral_bonus_days: totalBonusDays,
        referrals,
        stats: {
          total_referrals: referrals.length,
          adhered_count: adheredCount,
          activated_count: activatedCount,
          total_bonus_days: totalBonusDays
        }
      });
    } catch (err: any) {
      console.error("[Referral Info] Erro ao buscar dados de indicação:", err);
      return res.status(500).json({ error: err.message });
    }
  });

  // 6. Admin: List All Referral Leads & Adhesions
  app.get("/api/admin/referrals/leads", async (req, res) => {
    const session = await validateAdminSession(req);
    if ("error" in session) return res.status(session.status).json({ error: session.error });

    try {
      const leads = Array.from(referralLeadsStore.values()).sort((a, b) =>
        new Date(b.created_at).getTime() - new Date(a.created_at).getTime()
      );
  ...
  // 7. Admin: Activate Trial for Lead ("REGRA 15 + 15")
  app.post("/api/admin/referrals/:id/activate", async (req, res) => {
    const session = await validateAdminSession(req);
    if ("error" in session) return res.status(session.status).json({ error: session.error });
    const { adminProfile } = session;
    const { id } = req.params;
    ...
    try {
      const lead = referralLeadsStore.get(id);
      if (!lead) {
        return res.status(404).json({ error: "Registro de adesão não encontrado." });
      }

      // --- ANTIFRAUDE & SECURITY VALIDATIONS ---
      // 1. Idempotency Check: Prevent duplicate activation & double bonus
      if (lead.status === 'TESTE_ATIVADO') {
        return res.status(400).json({
          error: "Este teste já foi ativado anteriormente. O bônus de 15 dias já foi computado e não pode ser concedido novamente."
        });
      }

      // 2. Anti-Self-Referral Check: Prevent síndico from referring own condominium or same phone
      if (lead.referrer_condo_id) {
        const { data: refCondo } = await supabaseAdmin
          .from('condominiums')
          .select('*')
          .eq('id', lead.referrer_condo_id)
          .maybeSingle();

        if (refCondo) {
          const cleanRefPhone = (refCondo.phone || refCondo.manager_phone || '').replace(/\D/g, '');
          const cleanLeadPhone = lead.phone.replace(/\D/g, '');
          const cleanRefName = refCondo.name.toLowerCase().trim();
          const cleanLeadCondoName = lead.condo_name.toLowerCase().trim();

          if (cleanRefPhone && cleanLeadPhone && cleanRefPhone === cleanLeadPhone) {
            return res.status(400).json({
              error: "Tentativa de autoindicação detectada (mesmo telefone do condomínio indicador). Ação bloqueada pelo sistema de segurança."
            });
          }

          if (cleanRefName === cleanLeadCondoName) {
            return res.status(400).json({
              error: "Tentativa de autoindicação detectada (mesmo nome de condomínio). Ação bloqueada pelo sistema de segurança."
            });
          }
        }
      }

      // --- EXECUTE 15 + 15 BUSINESS RULE ---
      const now = new Date();
      const trial15DaysLater = new Date(now.getTime() + 15 * 24 * 60 * 60 * 1000).toISOString();

      // 1. Create or Activate Condomínio B (Indicado) with 15 test days
      let targetCondoB: any = null;

      // Check if condo already created in DB with matching name
      const { data: existingCondos } = await supabaseAdmin
        .from('condominiums')
        .select('*')
        .ilike('name', lead.condo_name)
        .maybeSingle();

      if (existingCondos) {
        targetCondoB = existingCondos;
        await supabaseAdmin.from('condominiums').update({
          trial_started_at: now.toISOString(),
          trial_ends_at: trial15DaysLater,
          plan_status: 'trial',
          active: true
        }).eq('id', targetCondoB.id);

        condoTrialStore.set(targetCondoB.id, {
          trial_started_at: now.toISOString(),
          trial_ends_at: trial15DaysLater,
          plan_status: 'trial',
          referral_code: targetCondoB.referral_code || generateReferralCodeForCondo(targetCondoB.name, targetCondoB.id),
          referral_bonus_days: 0
        });
      } else {
        // Create new condominium for B
        const newCondoId = crypto.randomUUID();
        const { data: createdCondo, error: createCondoErr } = await supabaseAdmin
          .from('condominiums')
          .insert([{
            id: newCondoId,
            name: lead.condo_name,
            address: req.body.address || 'Endereço a cadastrar',
            city: req.body.city || 'Cidade',
            state: req.body.state || 'UF',
            phone: lead.phone,
        ...
        if (createCondoErr) {
          console.warn("[Activation] Erro ao criar condomínio no DB (pode usar mock/store):", createCondoErr.message);
          targetCondoB = {
            id: newCondoId,
            name: lead.condo_name,
            manager_name: lead.name,
          ...
          targetCondoB = createdCondo;
        }

        // Initialize trial for Condo B
        condoTrialStore.set(targetCondoB.id, {
          trial_started_at: now.toISOString(),
          trial_ends_at: trial15DaysLater,
          plan_status: 'trial',
          referral_code: generateReferralCodeForCondo(targetCondoB.name, targetCondoB.id),
          referral_bonus_days: 0
        });
      }

      // 2. Grant +15 Bonus Days to Condomínio A (Indicador)
      let bonusGranted = false;
      let condoAUpdatedTrial: any = null;

      if (lead.referrer_condo_id) {
        const condoAId = lead.referrer_condo_id;
        const { data: condoA } = await supabaseAdmin
          .from('condominiums')
          .select('*')
          .eq('id', condoAId)
          .maybeSingle();

        const condoAName = condoA?.name || lead.referrer_condo_name || 'Condomínio Indicador';
        const currentTrialA = await getOrInitCondoTrial(condoAId, condoAName);

        const currentEndsA = new Date(currentTrialA.trial_ends_at).getTime();
        const baseTimeA = currentEndsA > now.getTime() ? currentEndsA : now.getTime();
        const newEndsA = new Date(baseTimeA + 15 * 24 * 60 * 60 * 1000).toISOString();
        const newBonusDaysA = (currentTrialA.referral_bonus_days || 0) + 15;

        condoAUpdatedTrial = {
          ...currentTrialA,
          trial_ends_at: newEndsA,
          referral_bonus_days: newBonusDaysA
        };
        condoTrialStore.set(condoAId, condoAUpdatedTrial);

        // Update DB for Condo A
        await supabaseAdmin.from('condominiums').update({
          trial_ends_at: newEndsA,
          referral_bonus_days: newBonusDaysA
        }).eq('id', condoAId);
        ...
        // Log BONUS_INDICACAO_CONCEDIDO in auditoria_eventos
        await safeLogAuditoria({
          condominio_id: condoAId,
          usuario_id: adminProfile.id,
          usuario_nome: adminProfile.full_name,
          usuario_perfil: adminProfile.role,
          tipo_evento: 'BONUS_INDICACAO_CONCEDIDO',
          acao: 'UPDATE',
          tabela_afetada: 'condominiums',
          registro_id: condoAId,
          descricao: `Bônus de indicação concedido: +15 DIAS GRÁTIS adicionados ao ${condoAName} pela ativação do condomínio indicado ${lead.condo_name}.`,
          metodo: 'ADMIN_ACTIVATION',
          dados_antes: currentTrialA,
          dados_depois: condoAUpdatedTrial
        });
      }

      // 3. Update Lead Status in Store
      lead.status = 'TESTE_ATIVADO';
      lead.bonus_granted = bonusGranted;
      lead.bonus_days_granted = bonusGranted ? 15 : 0;
      lead.activated_at = now.toISOString();
      lead.condo_created_id = targetCondoB.id;
      referralLeadsStore.set(id, lead);

      // Log TESTE_ATIVADO in auditoria_eventos
      await safeLogAuditoria({
        condominio_id: targetCondoB.id,
        usuario_id: adminProfile.id,
        usuario_nome: adminProfile.full_name,
        usuario_perfil: adminProfile.role,
        tipo_evento: 'TESTE_ATIVADO',
        acao: 'INSERT',
        tabela_afetada: 'referrals',
        registro_id: id,
        descricao: `Período de teste de 15 dias ativado pelo Administrador para o condomínio ${lead.condo_name} (Síndico ${lead.name}).${bonusGranted ? ` Bônus de +15 dias concedido ao indicador ${lead.referrer_condo_name}.` : ''}`,
        metodo: 'ADMIN_ACTIVATION',
        dados_depois: {
          lead_id: id,
          condominio_b_id: targetCondoB.id,
          condominio_b_nome: lead.condo_name,
          trial_starts: now.toISOString(),
          trial_ends: trial15DaysLater,
          bonus_granted: bonusGranted,
          referrer_condo: lead.referrer_condo_name || null
        }
      });

      return res.json({
        success: true,
        message: `Teste de 15 dias ativado com sucesso para "${lead.condo_name}"!${bonusGranted ? ` +15 dias de bônus creditados para "${lead.referrer_condo_name}".` : ''}`,
        lead,
        condoB: targetCondoB,
        condoA_updated: condoAUpdatedTrial,
        bonus_granted: bonusGranted,
        bonus_days: bonusGranted ? 15 : 0
      });
    } catch (err: any) {
      console.error("[Activate Referral] Erro ao ativar teste:", err);
      return res.status(500).json({ error: err.message || "Erro interno ao ativar teste." });
    }
  });

  // 8. Test / Simulation: Generate simulated visitor interest
  app.post("/api/admin/referrals/simulate-interest", async (req, res) => {
    const session = await validateAdminSession(req);
    if ("error" in session) return res.status(session.status).json({ error: session.error });

    const { name, phone, condo_name, ref, token } = req.body;
    const cleanRef = typeof ref === 'string' ? ref.trim().toUpperCase() : '';
    const cleanToken = typeof token === 'string' ? token.trim().toUpperCase() : '';
    ...
    const leadRecord: ReferralLeadRecord = {
      id: leadId,
      name: name || 'Síndico Convidado',
      phone: phone || '(11) 98888-7777',
      condo_name: condo_name || 'Condomínio Morada Nova',
      origin_type: cleanRef ? 'SINDICO' : 'ADMINISTRADOR',
      token_or_ref: cleanRef || cleanToken || 'DEMO-OFICIAL',
      status: 'INTERESSE',
      bonus_granted: false,
      bonus_days_granted: 0,
